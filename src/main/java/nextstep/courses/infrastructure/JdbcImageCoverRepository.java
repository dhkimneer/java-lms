...
public class JdbcImageCoverRepository implements ImageCoverRepository {
    ...
    @Override
    public Optional<ImageCovers> findBySessionId(Long sessionId) {
        ...
        List<ImageCover> result = jdbcTemplate.query(sql, rowMapper, sessionId);

        return result.isEmpty()
                ? Optional.empty()
                : Optional.of(new ImageCovers(result));
    }
}
