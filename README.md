...
## [4단계] - 요구사항 추가 (my)
1. 요구사항 정리
   1) 강의 진행 상태(준비 중, 진행 중, 종료)와 모집 상태(비모집 중, 모집 중)으로 상태 값을 분리한다.
     - 강의가 진행 중일 때도 수강 신청이 가능하다.
     -> (모집 중 & !종료)일 때 수강신청이 가능하다.
   2) 강의는 하나 이상의 커버 이미지를 가질 수 있다.
   3) 강사는 수강신청한 사람들 중 선발된 인원에 대해서만 수강 승인이 가능해야 한다.
   4) 강사는 수강신청한 사람들 중 선발되지 않은 인원에 대해서 수강 취소할 수 있어야 한다.
     -> 선발 행위를 승인을 통해 처리 / 취소 처리
   
   - 인원 여러 명을 선발 체크하고 그 인원들의 상태 값을 바꿔 저장하면 됨
   - 그 단계까지는 가지 않고, 단일 인원(Participant)에 대해 승인/취소 처리하는 것 (어차피 stream 도는 것 차이 밖에 없음)

2. 학습
   - 기존 Participant에 대한 명확한 정의 필요성 (Participant: 수강신청자)
     -> 그래서 승인 대기 중(PENDING), 승인(APPROVED), 취소(DISAPPROVED)로 나눔
   - Participant가 원래 원시 객체였으나, 추가 요구 사항으로 인해 자연스럽게 도메인 객체로 변경
   - 명확한 책임 분리
     -> Session(강의)은 Participants에 대해 신경 쓸 이유가 없음
     -> 강의는 강의대로 처리하고, Participant 관련 요구가 생길 경우 해당 객체에 요청하면 됨
     -> 조합은 서비스 단에서 하면 됨
